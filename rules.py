...
def total_revenue(data: dict, financial_index):
    ...
    """
    return data.get("financials")[financial_index].get("pnl").get("lineItems").get("netRevenue", 0.0)
...
def total_borrowing(data: dict, financial_index):
    ...
    """
    total_borrowings = data.get("financials")[financial_index].get("bs").get("lineItems").get("longTermBorrowings", 0.0) + \
                       data.get("financials")[financial_index].get("bs").get("lineItems").get("shortTermBorrowings", 0.0)

    total_revenue_value = total_revenue(data, financial_index)

    
    return total_borrowings / total_revenue_value if total_revenue_value != 0 else 0.0
...
def iscr(data: dict, financial_index):
    ...
    """
    profit_before_interest_tax_depreciation = data.get("financials")[financial_index].get("pnl").get("lineItems").get("profitBeforeInterestTaxDepreciation", 0.0)
    interest_expenses = data.get("financials")[financial_index].get("pnl").get("lineItems").get("interestExpenses", 0.0)

    
    return (profit_before_interest_tax_depreciation + 1) / (interest_expenses + 1) if interest_expenses != 0 else 0.0
